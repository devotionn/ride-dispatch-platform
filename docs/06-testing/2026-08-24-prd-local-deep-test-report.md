...
| 局域网入口 | `<LAN_IP>:8081/5173/5174` 和 Vite API 代理均 HTTP 200 |
...
| 公网 APK 构建 | `app-debug.apk` 使用文档示例 API `http://203.0.113.10` 表示外部验收地址，安装到 Pixel 7 Emulator 并启动 `MainActivity`；启动后无 `AndroidRuntime` 崩溃 |
...
- （历史记录，已被当前无商业地图架构取代）高德真实 Key 下的地图加载、搜索、定位和权限流程。
- 另一台局域网设备实际访问（当前只验证本机通过 Wi-Fi 地址访问）。
- 公网 80 已验证；非标准管理端端口在部分网络环境下可能受限，长期建议域名 HTTPS 443。
